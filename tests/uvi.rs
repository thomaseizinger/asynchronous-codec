use asynchronous_codec::{Framed, UviCodec};
...
fn same_msgs_are_received_as_were_sent() {
    ...
    let mut framed = Framed::new(cur, UviCodec);

    let send_msgs = async {
        framed.send(b"msg1").await.unwrap();
        framed.send(b"msg2").await.unwrap();
        framed.send(b"msg3").await.unwrap();
    };
    executor::block_on(send_msgs);

    let mut cur = framed.into_inner();
    cur.set_position(0);
    let framed = Framed::new(cur, UviCodec);
